静态导入是什么？

## 当前代码不足之处

缺一个异常处理框架，比如：我校验某一个字段抛出一个异常，返回的是500，但这种情况应该返回400

缺少数据校验框架：数据进入之后一般都是手动写代码进行校验，缺乏灵活性

缺少dto：即web客户端和后台交互，后台返回的数据有很多的冗余，会暴露后台数据结构

缺少压测/性能分析

缺少校验

Java 

Markdown UI 

sparrow-markdown

collapsing merge tree引擎有什么特点？



```
EventAnalysisOperation

AnalysisIndicatorType
```

## SQL
```sql
	SELECT 'total', groupArray(`$lastAppVersion,$lastCity,sc`),SUM(sc) 
    from (
    select  array($lastAppVersion,$lastCity,toString(sc)) as `$lastAppVersion,$lastCity,sc`, sum($sessionCount) as sc
    from app_insight_new.app_user
    where appIndex = 306 and sign > 0
    group by $lastAppVersion,$lastCity order by $lastAppVersion,$lastCity desc)
    
    UNION ALL 
    
    SELECT province,groupArray(`$lastAppVersion,$lastCity,sc`),SUM(sc) 
from (
	select $lastProvince as province, array($lastAppVersion,$lastCity,toString(sc)) as `$lastAppVersion,$lastCity,sc`, sum($sessionCount) as sc
    from app_insight_new.app_user
    where appIndex = 306 and sign > 0
    group by province,$lastAppVersion,$lastCity order by province,$lastAppVersion,$lastCity desc
    )
    group BY province order BY province desc
```

## 条件提交接口请求

### 一个指标/一个过滤条件/两个维度分组

```java
{
    "indicators": [
        {
            "propName": "会话次数",
            "op": "",
            "propId": "$sessionCount",
            "propValType": 1,
            "type": 0
        }
    ],
    "global": {
        "relationType": 1,
        "filters": [{
            "propId":"$lastAppVersion",
            "propName":"最近应用版本",
            "propValType":0,
            "selector":8,
            "payload":["1.0"]
        }
        ]
    },
    "dimensions": [{
        "propId": "$lastAppVersion",
        "propName":"最近应用版本",
        "propValType":0
    },
    {
        "propId": "$lastCity",
        "propName":"最近城市",
        "propValType":0
    }],
    "isMajorDimensionAnalysis":true,
    "pageSize":10,
    "pageIndex":1
}
```

- [x] 适用维度分支

- [x] 适用非维度分支都可以请求

### 一个指标/一个过滤条件/无维度分组

```json
{
    "indicators": [
        {
            "propName": "会话次数",
            "op": "",
            "propId": "$sessionCount",
            "propValType": 1,
            "type": 0
        }
    ],
    "global": {
        "relationType": 1,
        "filters": [{
            "propId":"$lastAppVersion",
            "propName":"最近应用版本",
            "propValType":0,
            "selector":8,
            "payload":["1.0"]
        }
        ]
    },
    "dimensions": [],
    "isMajorDimensionAnalysis":true,
    "pageSize":10,
    "pageIndex":1
}
```

- [ ] 适用维度分支

- [x] 适用非维度分支都可以请求

### 一个指标/一个过滤条件/一个维度分组

```json
{
    "indicators": [
        {
            "propName": "会话次数",
            "op": "",
            "propId": "$sessionCount",
            "propValType": 1,
            "type": 0
        }
    ],
    "global": {
        "relationType": 1,
        "filters": [{
            "propId":"$lastAppVersion",
            "propName":"最近应用版本",
            "propValType":0,
            "selector":8,
            "payload":["1.0"]
        }
        ]
    },
    "dimensions": [{
        "propId": "$lastAppVersion",
        "propName":"最近应用版本",
        "propValType":0
    }
    ],
    "isMajorDimensionAnalysis":true,
    "pageSize":10,
    "pageIndex":1
}
```

- [ ] 适用维度分支

- [x] 适用非维度分支请求

### 一个指标/多个过滤条件/三个维度

```json
{
    "indicators": [
        {
            "propName": "会话次数",
            "propId": "$sessionCount",
            "propValType": 1,
            "type": 0
        }
    ],
    "global": {
        "relationType": 1,
        "filters": [{
            "propId":"$lastAppVersion",
            "propName":"最近应用版本",
            "propValType":0,
            "selector":8,
            "payload":["1.0"]
        }
        ]
    },
    "dimensions": [{
        "propId": "$lastProvince",
        "propName":"最近省份",
        "propValType":0
    },{
        "propId": "$lastManufacturer",
        "propName":"最近设备厂商",
        "propValType":0
    },{
        "propId": "$isMultipleDevicesUser",
        "propName":"是否是多设备用户",
        "propValType":2
    }  
    ],
    "isMajorDimensionAnalysis":false,
    "pageSize":10,
    "pageIndex":1
}
```





```json
{
    "data": [
        {
            "mainDimValue": '主维度', 
            sum: 123,
            groupArray: [
                {
                    noMainDimValues: [
                         '111',
                         '222'
                    ],
                    sum: 333
                }
            ]
        }
    ],
    "mainDimesion": {
        "propId": 'asdf',
        "propValues": [12]
    },
    "noMainDimesion": [
        {
            "propId": 'propId',
            "propValues": [12]
        }
    ]
}
```





```
JsonObject mainDimVO = resultVO.getAsJsonObject("mainDimension");
mainDimVO.addProperty("propId", dimensions.get(0).getPropId());
mainDimVO.add("propValues", mainDimValues);
```
